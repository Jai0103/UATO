...
function drawHeader(
  ...
  if (logo) {
    const ratio =
      logo.naturalWidth / Math.max(logo.naturalHeight, 1);
...
export async function createUaMaintenancePdf(
...
export function uaMaintenancePdfFileName(
  record: UaMaintenanceRecord
) {
  const model = safeFileName(
    record.uaModel || "UA"
  );

  const uaId = safeFileName(
    record.uaId || "Maintenance"
  );
