...
import {
  analyzeApprovalsMigration,
  loadGoogleApprovalsSource,
  migrateApprovalsToFirestore,
  verifyApprovalsMigration,
  type ApprovalsMigrationAnalysis,
  type ApprovalsMigrationVerification
} from "@/lib/approvals-firebase-migration";
...
export default function ApprovalsMigrationPage() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [admin, setAdmin] = useState<ConnectedAdmin | null>(null);
  const [analysis, setAnalysis] = useState<ApprovalsMigrationAnalysis | null>(null);
  const [verification, setVerification] =
    useState<ApprovalsMigrationVerification | null>(null);
  ...
  async function analyzeSource() {
    ...
    setRunId("");
    try {
      setAnalysis(analyzeApprovalsMigration(await loadGoogleApprovalsSource()));
    } catch (analysisError) {
  ...
  async function copySource() {
    ...
    setVerification(null);
    try {
      const result = await migrateApprovalsToFirestore(analysis, {
        uid: admin.uid,
        email: admin.email
      });
  ...
  async function verifySource() {
    ...
    setVerification(null);
    try {
      setVerification(await verifyApprovalsMigration(analysis));
  ...
  const blocked = Boolean(
    analysis?.invalidItems.length || analysis?.duplicateItems.length
  );
      ...
      <div className="mx-auto w-full max-w-3xl">
        <section className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/30">
          <header className="border-b border-slate-700 px-6 py-6 sm:px-8">
            <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-sky-400/10 text-sky-300">
              <FileCheck2 className="h-5 w-5" />
            </div>
            <p className="text-xs font-bold uppercase text-sky-300">
              Administrator migration utility
            </p>
            <h1 className="mt-2 text-2xl font-bold text-white">AGA Approvals</h1>
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Copy approvals, permitted locations, expiry and renewal data, and
              Google Drive document references without changing the live workflow.
            </p>
          </header>
          ...
          {!admin ? (
            <form className="space-y-5 px-6 py-6 sm:px-8" onSubmit={connectFirebase}>
              ...
              <button
                className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-sky-400 font-bold text-slate-950 disabled:opacity-60"
                disabled={busy}
                type="submit"
              >
            ...
            <div className="space-y-5 px-6 py-6 sm:px-8">
              ...
              <button
                className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-slate-600 bg-slate-800 font-bold hover:border-sky-400 disabled:opacity-60"
                disabled={busy}
                onClick={analyzeSource}
                type="button"
              >
                {busy && !analysis ? <Loader2 className="h-5 w-5 animate-spin" /> : <DatabaseZap className="h-5 w-5" />}
                Analyze Google Sheets AGA Approvals
              </button>
              ...
              {analysis ? (
                <div className="rounded-xl border border-slate-700 bg-slate-950 p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <Stat label="Approvals" value={analysis.recordCount} />
                    <Stat label="Active approvals" value={analysis.activeRecordCount} />
                    <Stat label="Archived approvals" value={analysis.archivedRecordCount} />
                    <Stat label="Locations" value={analysis.locationCount} />
                    <Stat label="Active locations" value={analysis.activeLocationCount} />
                    <Stat label="Documents" value={analysis.documentCount} />
                    <Stat label="Current documents" value={analysis.currentDocumentCount} />
                  </div>

                  <div className="mt-4 rounded-lg border border-sky-400/30 bg-sky-400/10 p-4 text-sm text-sky-100">
                    Approval PDFs remain in Google Drive. Firestore stores their
                    file IDs and Drive links, so Firebase Storage is not required.
                  </div>
                  ...
                  {blocked ? (
                    <div className="mt-4 rounded-lg border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">
                      ...
                      {[...analysis.invalidItems, ...analysis.duplicateItems].map((item) => (
                  ...
                  <button
                    className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-sky-400 font-bold text-slate-950 disabled:opacity-50"
                    disabled={busy || blocked}
                    onClick={copySource}
                    type="button"
                  >
                    {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <DatabaseZap className="h-5 w-5" />}
                    Copy AGA Approvals to Firestore
                  </button>
                  <button
                    className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-sky-400/40 bg-sky-400/10 font-bold text-sky-100 disabled:opacity-50"
                    disabled={busy || blocked}
          ...
          {runId ? (
            <Notice
              tone="success"
              text={"AGA Approvals migration completed. Run ID: " + runId}
            />
          ) : null}
          {verification ? (
            <div
...
function Field({
  ...
  return (
    <label className="block">
      ...
      <input
        className="mt-2 h-12 w-full rounded-xl border border-slate-600 bg-slate-950 px-4 outline-none focus:border-sky-400"
