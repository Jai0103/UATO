      ...
      <div className="mx-auto max-w-xl">
        ...
        {!loading && session ? <>
          ...
          {!success && periodOpen && scheduleAllowsPeriod ? <form onSubmit={handleSubmit} className="mt-5 space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <label className="block text-sm font-semibold text-slate-700">Last 4 characters of NRIC / FIN / travel document<input inputMode="text" autoCapitalize="characters" maxLength={4} value={lastFour} onChange={(event) => setLastFour(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))} className="mt-2 h-12 w-full rounded-lg border border-slate-300 bg-white px-3.5 text-base font-bold uppercase tracking-[0.2em] outline-none focus:border-cyan-700 focus:ring-4 focus:ring-cyan-100" placeholder="123A" /></label>
