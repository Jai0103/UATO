...
import {
  analyzeStaffTrainingMigration,
  loadGoogleStaffTrainingSource,
  migrateStaffTrainingToFirestore,
  verifyStaffTrainingMigration,
  type StaffTrainingMigrationAnalysis,
  type StaffTrainingMigrationVerification
} from "@/lib/staff-training-firebase-migration";
...
export default function StaffTrainingMigrationPage() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [admin, setAdmin] = useState<ConnectedAdmin | null>(null);
  const [analysis, setAnalysis] = useState<StaffTrainingMigrationAnalysis | null>(null);
  const [verification, setVerification] = useState<StaffTrainingMigrationVerification | null>(null);
  ...
  async function analyzeSource() {
    ...
    try {
      setAnalysis(
        analyzeStaffTrainingMigration(await loadGoogleStaffTrainingSource())
      );
    } catch (analysisError) {
  ...
  async function copySource() {
    ...
    setVerification(null);
    try {
      const result = await migrateStaffTrainingToFirestore(analysis, {
        uid: admin.uid,
        email: admin.email
      });
  ...
  async function verifySource() {
    ...
    setVerification(null);
    try {
      setVerification(await verifyStaffTrainingMigration(analysis));
      ...
      <div className="mx-auto w-full max-w-3xl">
        <section className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/30">
          <header className="border-b border-slate-700 px-6 py-6 sm:px-8">
            ...
            <p className="text-xs font-bold uppercase text-emerald-300">Administrator migration utility</p>
            <h1 className="mt-2 text-2xl font-bold text-white">Staff Internal Training</h1>
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Copy training descriptions, staff records, checklist results, remarks, and signatures without changing Google Sheets.
            </p>
          </header>
          ...
          {!admin ? (
            <form className="space-y-5 px-6 py-6 sm:px-8" onSubmit={connectFirebase}>
              ...
              <button className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 font-bold text-slate-950 disabled:opacity-60" disabled={busy} type="submit">
            ...
            <div className="space-y-5 px-6 py-6 sm:px-8">
              ...
              <button className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-slate-600 bg-slate-800 font-bold hover:border-emerald-400 disabled:opacity-60" disabled={busy} onClick={analyzeSource} type="button">
                {busy && !analysis ? <Loader2 className="h-5 w-5 animate-spin" /> : <DatabaseZap className="h-5 w-5" />}
                Analyze Google Sheets Staff Training
              </button>
              ...
              {analysis ? (
                <div className="rounded-xl border border-slate-700 bg-slate-950 p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <Stat label="Descriptions" value={analysis.descriptionCount} />
                    <Stat label="Staff records" value={analysis.recordCount} />
                    <Stat label="Checklist entries" value={analysis.entryCount} />
                    <Stat label="Completed entries" value={analysis.completedEntryCount} />
                  ...
                  <button className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 font-bold text-slate-950 disabled:opacity-50" disabled={busy || blocked} onClick={copySource} type="button">
                    {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <DatabaseZap className="h-5 w-5" />}
                    Copy Staff Training to Firestore
                  </button>
                  <button className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-emerald-400/40 bg-emerald-400/10 font-bold text-emerald-100 disabled:opacity-50" disabled={busy || blocked} onClick={verifySource} type="button">
          ...
          {runId ? <Notice tone="success" text={`Staff Training migration completed. Run ID: ${runId}`} /> : null}
          {verification ? (
            <div className={`m-6 rounded-xl border p-4 text-sm sm:m-8 ${verification.verified ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-100" : "border-amber-400/30 bg-amber-400/10 text-amber-100"}`}>
              <p className="font-bold">{verification.verified ? "Firestore Staff Training verified exactly." : "Verification found differences."}</p>
...
function Field({ label, type, value, onChange }: { label: string; type: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block">
      ...
      <input className="mt-2 h-12 w-full rounded-xl border border-slate-600 bg-slate-950 px-4 outline-none focus:border-emerald-400" onChange={(event) => onChange(event.target.value)} required type={type} value={value} />
    </label>
  );
}
...
function Notice({ tone, text }: { tone: "error" | "success"; text: string }) {
  ...
  return (
    <div className={`m-6 flex gap-3 rounded-xl border p-4 text-sm sm:m-8 ${error ? "border-rose-400/30 bg-rose-400/10 text-rose-100" : "border-emerald-400/30 bg-emerald-400/10 text-emerald-100"}`} role={error ? "alert" : undefined}>
