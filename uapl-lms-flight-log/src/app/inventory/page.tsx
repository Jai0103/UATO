...
export default function InventoryPage() {
  ...
  const skipNextAssetFilterLoad = useRef(true);
  const skipNextActivityFilterLoad = useRef(true);
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      try {
        if (routeMode === "assets") {
          skipNextAssetFilterLoad.current = true;
          const [master, stats, assets] = await Promise.all([
            fetchInventoryMasterData(),
            fetchInventoryDashboard(),
        ...
        if (routeMode === "activity") {
          skipNextActivityFilterLoad.current = true;
          const activities = await fetchInventoryActivityPage({
            page: 1,
            pageSize: 10
          });
          if (requestId !== initializeSequence.current) return;
        ...
        const master = await fetchInventoryMasterData();
        if (requestId !== initializeSequence.current) return;
        setMasterData(master);
      } catch (error) {
        if (requestId !== initializeSequence.current) return;
    ...
    return () => {
      ...
      tableRequestSequence.current += 1;
    };
  ...
  useEffect(() => {
    ...
    if (skipNextAssetFilterLoad.current) {
      skipNextAssetFilterLoad.current = false;
      return;
    }
  ...
  useEffect(() => {
    ...
    if (skipNextActivityFilterLoad.current) {
      skipNextActivityFilterLoad.current = false;
      return;
    }
  ...
  async function loadAssets(page = assetPage.page) {
    const requestId = ++tableRequestSequence.current;
    const request = {
      page,
      pageSize: 10,
      query,
    ...
    try {
      const result = await fetchInventoryAssetsPage(request);
      ...
      if (requestId === tableRequestSequence.current) setTableLoading(false);
    }
  }
  ...
  async function loadActivities(page = activityPage.page) {
    ...
    setTableLoading(true);
    try {
      const result = await fetchInventoryActivityPage(request);
      if (requestId !== tableRequestSequence.current) return;
