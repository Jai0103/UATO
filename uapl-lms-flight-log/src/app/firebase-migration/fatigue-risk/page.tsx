...
import {
  analyzeFatigueRiskMigration,
  loadGoogleFatigueRiskSource,
  migrateFatigueRiskToFirestore,
  verifyFatigueRiskMigration,
  type FatigueRiskMigrationAnalysis,
  type FatigueRiskMigrationVerification
} from "@/lib/fatigue-risk-firebase-migration";
...
export default function FatigueRiskMigrationPage() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [admin, setAdmin] = useState<ConnectedAdmin | null>(null);
  const [analysis, setAnalysis] =
    useState<FatigueRiskMigrationAnalysis | null>(null);
  const [verification, setVerification] =
    useState<FatigueRiskMigrationVerification | null>(null);
  ...
  async function analyzeSource() {
    ...
    setRunId("");
    try {
      const records = await loadGoogleFatigueRiskSource();
      setAnalysis(analyzeFatigueRiskMigration(records));
    } catch (analysisError) {
  ...
  async function copySource() {
    ...
    setVerification(null);
    try {
      const result = await migrateFatigueRiskToFirestore(analysis, {
        uid: admin.uid,
        email: admin.email
      });
  ...
  async function verifySource() {
    ...
    setVerification(null);
    try {
      setVerification(await verifyFatigueRiskMigration(analysis));
      ...
      <div className="mx-auto w-full max-w-3xl">
        <section className="overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/30">
          <header className="border-b border-slate-700 px-6 py-6 sm:px-8">
            <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-rose-400/10 text-rose-300">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Copy weekly assessments, responses, recommendations, review
              status, and signatures without changing Google Sheets.
            </p>
          </header>
          ...
          {!admin ? (
            ...
            >
              ...
              />
              <button
                className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-rose-400 font-bold text-slate-950 disabled:opacity-60"
                disabled={busy}
                type="submit"
              >
                {busy ? (
            ...
            <div className="space-y-5 px-6 py-6 sm:px-8">
              ...
              <button
                className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-slate-600 bg-slate-800 font-bold hover:border-rose-400 disabled:opacity-60"
                disabled={busy}
              ...
              >
                ...
                Analyze Google Sheets Fatigue Risk
              </button>
              ...
              {analysis ? (
                <div className="rounded-xl border border-slate-700 bg-slate-950 p-5">
                  ...
                  {blocked ? (
                    <div className="mt-4 rounded-lg border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">
                      ...
                      {[
                        ...
                        ...analysis.oversizedSignatures.map(
                          (item) => "Oversized signature: " + item
                        )
                      ].map((item) => (
                  ...
                  <button
                    className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-rose-400 font-bold text-slate-950 disabled:opacity-50"
                  ...
                  >
                    ...
                    Copy Fatigue Risk to Firestore
                  </button>
                  <button
                    className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-rose-400/40 bg-rose-400/10 font-bold text-rose-100 disabled:opacity-50"
                    disabled={busy || blocked}
          ...
          {runId ? (
            <Notice
              success
              text={"Fatigue Risk migration completed. Run ID: " + runId}
            />
          ) : null}
          {verification ? (
            <div
            ...
            >
              <p className="font-bold">
                {verification.verified
                  ? "Firestore Fatigue Risk verified exactly."
...
function Input({
  ...
  return (
    <label className="block">
      ...
      <input
        className="mt-2 h-12 w-full rounded-xl border border-slate-600 bg-slate-950 px-4 outline-none focus:border-rose-400"
