...
const UAPL_LMS_LOGO_URL =
  "https://jai0103.github.io/UATO/AGA_Logo_fullcolor_Horizontal%20(1).png";
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "setupFatigueRisk") {
      requireSession(payload, ["admin"]);
      return jsonResponse(setupFatigueRisk());
    }

    if (action === "getFatigueRiskRecordsPage") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        ...getFatigueRiskRecordsPage(payload, session)
      });
    }

    if (action === "getFatigueRiskWeeklyStatus") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        status: getFatigueRiskWeeklyStatus()
      });
    }

    if (action === "getFatigueRiskRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: getFatigueRiskRecord(payload, session)
      });
    }

    if (action === "saveFatigueRiskRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: saveFatigueRiskRecord(payload, session)
      });
    }

    if (action === "deleteFatigueRiskRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        recordId: deleteFatigueRiskRecord(payload, session)
      });
    }

    if (action === "getStaffTrainingDescriptions") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        descriptions: getStaffTrainingDescriptions()
      });
    }

    if (action === "getStaffTrainingRecords") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        records: getStaffTrainingRecords()
      });
    }

    if (action === "getStaffTrainingRecordsPage") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        ...getStaffTrainingRecordsPage(payload)
      });
    }

    if (action === "getStaffTrainingRecord") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: getStaffTrainingRecord(payload.recordId)
      });
    }

    if (action === "saveStaffTrainingRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: saveStaffTrainingRecord(payload, session)
      });
    }

    if (action === "deleteStaffTrainingRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        recordId: deleteStaffTrainingRecord(payload, session)
      });
    }

    if (action === "getUaMaintenanceMasterData") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        masterData: getUaMaintenanceMasterData()
      });
    }

    if (action === "saveUaMaintenanceMasterData") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        masterData: saveUaMaintenanceMasterData(payload, session)
      });
    }

    if (action === "getUaMaintenanceRecordsPage") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        ...getUaMaintenanceRecordsPage(payload)
      });
    }

    if (action === "getUaMaintenanceRecord") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: getUaMaintenanceRecord(payload.recordId)
      });
    }

    if (action === "saveUaMaintenanceRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: saveUaMaintenanceRecord(payload, session)
      });
    }

    if (action === "deleteUaMaintenanceRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        recordId: deleteUaMaintenanceRecord(payload, session)
      });
    }

    if (action === "getBulkFlightReportRecords") {
      requireSession(payload, ["admin", "trainer"]);
      return jsonResponse({
        ok: true,
        success: true,
        records: getBulkFlightReportRecords(payload)
      });
    }

    if (action === "setupInventory") {
      requireSession(payload, ["admin"]);
      return jsonResponse(setupInventory());
    }

    if (action === "getInventoryMasterData") {
      requireSession(payload, ["admin"]);
      return jsonResponse({ ok: true, success: true, masterData: getInventoryMasterData() });
    }

    if (action === "saveInventoryMasterData") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        masterData: saveInventoryMasterData(payload, session)
      });
    }

    if (action === "getInventoryDashboard") {
      requireSession(payload, ["admin"]);
      return jsonResponse({ ok: true, success: true, dashboard: getInventoryDashboard() });
    }

    if (action === "getInventoryAssetsPage") {
      requireSession(payload, ["admin"]);
      return jsonResponse({ ok: true, success: true, ...getInventoryAssetsPage(payload) });
    }

    if (action === "getInventoryAsset") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        detail: getInventoryAsset(payload.assetId)
      });
    }

    if (action === "saveInventoryAsset") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        asset: saveInventoryAsset(payload, session)
      });
    }

    if (action === "archiveInventoryAsset") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        assetId: archiveInventoryAsset(payload, session)
      });
    }

    if (action === "saveInventoryTransaction") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        transaction: saveInventoryTransaction(payload, session)
      });
    }

    if (action === "saveInventoryMaintenance") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true, success: true,
        maintenance: saveInventoryMaintenance(payload, session)
      });
    }

    if (action === "getInventoryActivityPage") {
      requireSession(payload, ["admin"]);
      return jsonResponse({ ok: true, success: true, ...getInventoryActivityPage(payload) });
    }

    if (action === "getBulkStaffTrainingReportRecords") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        records: getBulkStaffTrainingReportRecords(payload)
      });
    }

    if (action === "getBulkUaMaintenanceReportRecords") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        records: getBulkUaMaintenanceReportRecords(payload)
      });
    }
    ...
    if (action === "setupApprovals") {
      requireSession(payload, ["admin"]);
      return jsonResponse(setupApprovals());
    }

    if (action === "getApprovalsPage") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        ...getApprovalsPage(payload)
      });
    }

    if (action === "getApprovalRecord") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: getApprovalRecord(payload.approvalId || payload.recordId)
      });
    }

    if (action === "getApprovalDashboardSummary") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        dashboard: getApprovalDashboardSummary()
      });
    }

    if (action === "saveApprovalRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: saveApprovalRecord(payload, session)
      });
    }

    if (action === "archiveApprovalRecord") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        record: archiveApprovalRecord(payload, session)
      });
    }

    if (action === "saveApprovalDocument") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        document: saveApprovalDocument(payload, session)
      });
    }

    if (action === "getApprovalDocumentFile") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        ...getApprovalDocumentFile(payload)
      });
    }

    if (action === "deleteApprovalDocument") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        documentId: deleteApprovalDocument(payload, session)
      });
    }
    ...
    if (action === "setupStaffTraining") {
      requireSession(payload, ["admin"]);
      return jsonResponse(setupStaffTraining());
    }

    if (action === "setupUaMaintenance") {
      requireSession(payload, ["admin"]);
      return jsonResponse(setupUaMaintenance());
    }

    if (action === "saveStaffTrainingDescriptions") {
      const session = requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        descriptions: saveStaffTrainingDescriptions(payload, session)
      });
    }
    ...
    if (action === "getAdminDashboardBundle") {
      requireSession(payload, ["admin"]);
      return jsonResponse({
        ok: true,
        success: true,
        bundle: getAdminDashboardBundle()
      });
    }
...
function getAdminDashboardBundle() {
  ...
  try {
    bundle.approvals.data = getApprovalDashboardSummary();
  } catch (error) {
    bundle.approvals.available = false;
  ...
  try {
    bundle.fatigueRisk.data = getFatigueRiskWeeklyStatus();
  } catch (error) {
    bundle.fatigueRisk.available = false;
...
function forgotPassword(payload) {
  ...
  const emailPurpose =
    String(payload.purpose || "password-reset") === "new-account"
      ? "new-account"
      : "password-reset";
  ...
  const isNewAccount = emailPurpose === "new-account";
  const emailTitle = isNewAccount
    ? "Your account is ready"
    : "Your password has been reset";
  const emailSubject = isNewAccount
    ? "Welcome to Apollo Flight Management System"
    : "Apollo Flight Management System password reset";
  const emailIntro = isNewAccount
    ? "An account has been created for you in the Apollo Flight Management System."
    : "A temporary password has been issued for your Apollo Flight Management System account.";
  const emailSecurityNote = isNewAccount
    ? "For security, you will be asked to create a new password when you first sign in."
    : "For security, you will be asked to create a new password after signing in.";
  ...
  const emailLogoBlob = getEmailLogoBlob_();
  const emailLogoSource = emailLogoBlob
    ? "cid:apolloGlobalAcademyLogo"
    : UAPL_LMS_LOGO_URL;
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;background-color:#f3f6fb;">
            <tr>
              <td align="center" style="padding:32px 16px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:600px;background-color:#ffffff;border:1px solid #dbe3ee;border-radius:8px;overflow:hidden;">
                  <tr><td style="height:5px;background-color:#0284c7;font-size:0;line-height:0;">&nbsp;</td></tr>
                  <tr>
                    <td align="center" style="padding:28px 28px 24px;border-bottom:1px solid #e2e8f0;">
                      <img src="${emailLogoSource}" width="230" alt="Apollo Global Academy" style="display:block;width:230px;max-width:100%;height:auto;border:0;">
                      <div style="margin-top:18px;font-size:12px;line-height:18px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:#0369a1;">
                  ...
                  <tr>
                    <td style="padding:30px 32px 12px;">
                      <div style="font-size:12px;line-height:18px;font-weight:700;letter-spacing:0.4px;text-transform:uppercase;color:#64748b;">
                        ${isNewAccount ? "Account invitation" : "Security notification"}
                      </div>
                      <h1 style="margin:8px 0 0;font-size:26px;line-height:34px;color:#0f172a;font-weight:700;">${emailTitle}</h1>
                      <p style="margin:18px 0 0;font-size:15px;line-height:24px;color:#475569;">Hello ${safeUserName},</p>
                      <p style="margin:10px 0 0;font-size:15px;line-height:24px;color:#475569;">${emailIntro}</p>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding:16px 32px;">
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border:1px solid #cbd5e1;border-radius:8px;background-color:#f8fafc;">
                        <tr>
                          <td style="padding:18px 20px;border-bottom:1px solid #e2e8f0;">
                            <div style="font-size:11px;line-height:16px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:#64748b;">Temporary password</div>
                            <div style="margin-top:7px;font-family:Consolas,'Courier New',monospace;font-size:24px;line-height:30px;font-weight:700;letter-spacing:1px;color:#0f172a;word-break:break-all;">${safePassword}</div>
                          </td>
                        </tr>
                        <tr>
                          <td style="padding:13px 20px;font-size:13px;line-height:20px;color:#64748b;">
                            Account role: <strong style="color:#334155;">${safeRole}</strong>
                          </td>
                        </tr>
                      </table>
                    </td>
                  ...
                  <tr>
                    <td align="center" style="padding:12px 32px 8px;">
                      <a href="${UAPL_LMS_SIGN_IN_URL}" target="_blank" style="display:inline-block;background-color:#0369a1;color:#ffffff;text-decoration:none;font-size:15px;line-height:20px;font-weight:700;padding:14px 28px;border-radius:8px;">
                  ...
                  <tr>
                    <td style="padding:16px 32px 30px;">
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;background-color:#f0f9ff;border-left:3px solid #38bdf8;">
                        <tr><td style="padding:13px 15px;font-size:13px;line-height:20px;color:#334155;">${emailSecurityNote}</td></tr>
                      </table>
                      <p style="margin:20px 0 0;font-size:12px;line-height:19px;color:#64748b;word-break:break-all;">
                        If the button does not open, use this link:<br>
                        <a href="${UAPL_LMS_SIGN_IN_URL}" style="color:#0369a1;text-decoration:underline;">${UAPL_LMS_SIGN_IN_URL}</a>
                      </p>
                      <p style="margin:14px 0 0;font-size:12px;line-height:19px;color:#64748b;">If you did not expect this email, contact your administrator immediately.</p>
                    </td>
                  </tr>
                  <tr>
                    <td align="center" style="padding:18px 24px;background-color:#0f172a;color:#cbd5e1;font-size:11px;line-height:18px;">
                      Apollo Global Academy &bull; UATO<br>
                      This is an automated account security message.
                    </td>
                  </tr>
                </table>
              </td>
  ...
  if (emailLogoBlob) {
    emailMessage.inlineImages = {
      apolloGlobalAcademyLogo: emailLogoBlob
    };
  }
  ...
  return jsonResponse({
    ...
    message:
      isNewAccount
        ? "The account invitation has been sent by email."
...
function getEmailLogoBlob_() {
  try {
    ...
    return response
      .getBlob()
      .setName(
        "apollo-global-academy-logo.png"
      );
  } catch (error) {
    console.warn(
      "Email logo could not be embedded: " +
        String(
          error && error.message
            ? error.message
            : error
